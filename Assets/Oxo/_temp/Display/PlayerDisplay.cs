...
public class PlayerDisplay : MonoBehaviour
{
    ...
    AnimatorDisplay animatorDisplay;

    #endregion

    private void Awake()
    {
        animatorDisplay = GetComponent<AnimatorDisplay>();
    ...
    void FingerDrag()
    {
        ...
        else if (mechanic == MechanicType.Joystick)
        {
            if (cross != null) //Character Control
            {
                ...
                //rigidbody.AddForce(transform.forward * movementVector);
    ...
    void SwipeLeft()
    {
        --currentIndex;
        // TakeThePosition();
        animatorDisplay.StartDodge(.5f);
    }

    void SwipeUp()
    {
        animatorDisplay.StartDodge(1f);

    }

    void SwipeDown()
    {
        animatorDisplay.StartSlide(1f);
    }
    ...
    void OnGameStart()
    {
        if (mechanic == MechanicType.Swerve)
        {
            animatorDisplay.ForwardMovement(movementSpeed);
        }
        else if (mechanic == MechanicType.Joystick)
        {
